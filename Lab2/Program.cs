...
using static NNBasics.NNBasicsLimak.Core.Utilities.UtilitiesFunctions;
...
namespace NNBasics.Lab2
{
   class Program
   {
      static void Main(string[] args)
      {
         ...
         var networkTask1a =
               NeuralNetwork.Builder.AttachPredictionLayer(new List<double>(new[] { 0.5 }).ToMatrix()
                  .ToOutputNeurons()).WithAlpha(0.1).ApplyTheNameOfYourNetwork("Lab2_Task1a").BuildNetwork();
         networkTask1a.Train(new List<double>(new[] { 0.8 }).ToMatrix(),
            new List<double>(new[] { 2.0 }).ToMatrix(), 100);

         var networkTask1b =
            NeuralNetwork.Builder.AttachPredictionLayer(new List<double>(new[] { 0.5 }).ToMatrix()
               .ToOutputNeurons()).WithAlpha(1).ApplyTheNameOfYourNetwork("Lab2_Task1b").BuildNetwork();
         networkTask1b.Train(new List<double>(new[] { 0.8 }).ToMatrix(),
            new List<double>(new[] { 2.0 }).ToMatrix(), 100);

         var networkTask1c =
            NeuralNetwork.Builder.AttachPredictionLayer(new List<double>(new[] { 0.5 }).ToMatrix()
               .ToOutputNeurons()).WithAlpha(1).ApplyTheNameOfYourNetwork("Lab2_Task1c").BuildNetwork();
         ...
         var networkTask2 =
            NeuralNetwork.Builder.AttachPredictionLayer(new List<double>(new[] { 0.1, 0.2, -0.1 }).ToMatrix()
               .ToOutputNeurons()).WithAlpha(0.01).ApplyTheNameOfYourNetwork("Lab2_Task2").BuildNetwork();
         ...
         var mat = new List<double>(new[] { 0.1, 0.1, -0.3 }).ToMatrix();
         mat.Add(new List<double> { 0.1, 0.2, 0.0 });
         mat.Add(new List<double> { 0.0, 1.3, 0.1 });

         var networkTask3 =
            NeuralNetwork.Builder.AttachPredictionLayer(mat.ToOutputNeurons()).WithAlpha(0.01).ApplyTheNameOfYourNetwork("Lab2_Task3").BuildNetwork();
         ...
         var networkTask4 =
            NeuralNetwork.Builder.AttachPredictionLayer(4,3, 0.2, -0.2).WithAlpha(0.01).ApplyTheNameOfYourNetwork("Lab2_Task4_Colors").BuildNetwork();
